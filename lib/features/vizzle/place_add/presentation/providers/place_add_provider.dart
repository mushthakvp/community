...
class PlaceAddProvider extends ChangeNotifier {
  ...
  Future<void> editAd({
    ...
    if (networkImages.isEmpty && _selectedImages.isEmpty) {
      _adCreationResult = Error(message: "Please add at least one image");
      notifyListeners();
      return;
    }

    if (_latitude == null || _longitude == null || _address == null) {
      _adCreationResult = Error(message: "Please select location");
      notifyListeners();
      return;
    }
    ...
    try {
      ...
      imageUploadResult.fold((failure) {
        ...
        notifyListeners();
        return;
      }, (urls) => newImageUrls = urls);

      // Combine existing and new images
      final allImages = [...networkImages, ...newImageUrls];
      final editNumberPrice = num.tryParse(editPrice);

      final request = AdCreationRequest(
        title: title,
        description: description,
        price: editNumberPrice?.toDouble(),
        phoneNumber: phone,
        ...
        longitude: double.tryParse(_longitude ?? '0') ?? 0,
        address: _address ?? '',
        additionalFields: {"id": id},
      );

      // For edit, we would need an EditAdUseCase - simplified for now
      _adCreationResult = Success(
      ...
      _adCreationResult = Error(message: 'Failed to edit ad: $e');
      publishAddStatus = 1;
    }
